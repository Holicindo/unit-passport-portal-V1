...
import ReworkReportForm from '@/components/reports/ReworkReportForm';
...
const EMPTY_FORM = {
  ...
  rework_items: Array(6).fill(null).map(() => ({ ...EMPTY_ROW })),
  penyebab: {
    human_error: false,
    material_defect: false,
    proses_tidak_sesuai: false,
    desain_spesifikasi: false,
    lainnya: false,
    lainnya_text: '',
  },
  verifikasi: {
    sesuai_spesifikasi: false,
    tidak_sesuai: false,
    ditolak: false,
  },
  catatan: '',
...
export default function ReworkFormPage() {
    ...
    <div className={styles.pageWrapper}>
      ...
      <div className={styles.toolbar}>
        ...
        <h1 className={styles.title}>
          {isEditMode ? 'EDIT' : 'NEW'} FORM PENGECEKAN REWORK
        </h1>
      ...
      <div className={styles.unitBar}>
        <div className={styles.unitBarInner}>
          <div className={styles.unitField}>
            <label>PILIH UNIT YANG DIREWORK</label>
            <CustomSelect
      ...
      >
        {showPreview
          ? <ReworkReportTemplate mode="view" data={form} unit={unit || {}} />
          : <ReworkReportForm data={form} onChange={handleChange} />}
      </div>
      ...
      {!showPreview && (
        <div className={styles.photoBar}>
          <div className={styles.sectionHeader}>
            <h3>Lampiran Foto Dokumentasi</h3>
          </div>
      ...
      <div className={styles.formFooter}>
        ...
        >
          ...
          {loading ? 'Menyimpan...' : (isEditMode ? 'Simpan Perubahan' : 'Simpan Form Pengecekan Rework')}
        </button>
      </div>
      ...
      <Modal
        ...
        title={isEditMode ? 'Simpan Perubahan?' : 'Simpan Form Pengecekan Rework?'}
      ...
      <Modal
        ...
        title={isEditMode ? 'Laporan Berhasil Diperbarui!' : 'Form Pengecekan Rework Berhasil Disimpan!'}
        message={`Laporan untuk serial number ${unit?.serial_number} telah berhasil ${isEditMode ? 'diperbarui' : 'dicatat'}.`}
        type="success"
      />
    </div>
  );
