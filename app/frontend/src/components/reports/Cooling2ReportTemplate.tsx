...
export default function Cooling2ReportTemplate({ mode = 'edit', data = {}, unit = {}, onChange }: any) {
  const isEdit = mode === 'edit';
  const setHeader = (k: string, v: any) => { if (onChange && isEdit) onChange('header', { ...data.header, [k]: v }); };
  const setTempRow = (row: 'cake' | 'rtd', k: string, v: any) => {
  ...
  return (
    <div className={`${styles.a4Sheet} ${styles.coolingReport}`}>
      ...
      {/* GENERAL INSPECTION — 2 rows: CAKE & RTD */}
      <table className={styles.denseTable} style={{ tableLayout: 'fixed' }}>
        ...
        <tbody>
          {/* CAKE row */}
          <tr>
            <td className={styles.centerCell} style={{ fontWeight: 'bold' }}>CAKE</td>
            <td>{isEdit ? <input type="text" className={styles.cellInput} placeholder="Part" value={gi.part || ''} onChange={e => { if (onChange && isEdit) onChange('general_inspection', { ...gi, part: e.target.value }); }} /> : (gi.part || '-')}</td>
            <td colSpan={2}>{isEdit ? <input type="text" className={styles.cellInput} value={cake.required || ''} onChange={e => setTempRow('cake', 'required', e.target.value)} /> : (cake.required || '-')}</td>
            <td colSpan={2} className={styles.centerCell}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {isEdit ? <input type="text" className={`${styles.cellInput} ${styles.centerCell}`} value={cake.setting_temp || ''} onChange={e => setTempRow('cake', 'setting_temp', e.target.value)} style={{ width: '40px' }} /> : <span>{cake.setting_temp || '-'}</span>}
                <span style={{ marginLeft: '2px' }}>°C</span>
              </div>
            </td>
            <td colSpan={2} className={styles.centerCell}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {isEdit ? <input type="text" className={`${styles.cellInput} ${styles.centerCell}`} value={cake.temp_dalam || ''} onChange={e => setTempRow('cake', 'temp_dalam', e.target.value)} style={{ width: '40px' }} /> : <span>{cake.temp_dalam || '-'}</span>}
                <span style={{ marginLeft: '2px' }}>°C</span>
              </div>
            </td>
            <td colSpan={2} className={styles.centerCell}>{isEdit ? <input type="text" className={`${styles.cellInput} ${styles.centerCell}`} value={cake.digital_type || ''} onChange={e => setTempRow('cake', 'digital_type', e.target.value)} /> : (cake.digital_type || '-')}</td>
            <td colSpan={5}>{isEdit ? <textarea className={styles.cellInput} style={{ minHeight: '30px', resize: 'vertical' }} value={cake.setting || ''} onChange={e => setTempRow('cake', 'setting', e.target.value)} /> : <div style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word', maxHeight: '60px', overflow: 'hidden', fontSize: '7px' }}>{cake.setting || '-'}</div>}</td>
          </tr>
          {/* RTD row */}
          <tr>
            <td className={styles.centerCell} style={{ fontWeight: 'bold' }}>RTD</td>
            <td></td>
            <td colSpan={2}>{isEdit ? <input type="text" className={styles.cellInput} value={rtd.required || ''} onChange={e => setTempRow('rtd', 'required', e.target.value)} /> : (rtd.required || '-')}</td>
            <td colSpan={2} className={styles.centerCell}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {isEdit ? <input type="text" className={`${styles.cellInput} ${styles.centerCell}`} value={rtd.setting_temp || ''} onChange={e => setTempRow('rtd', 'setting_temp', e.target.value)} style={{ width: '40px' }} /> : <span>{rtd.setting_temp || '-'}</span>}
                <span style={{ marginLeft: '2px' }}>°C</span>
              </div>
            </td>
            <td colSpan={2} className={styles.centerCell}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {isEdit ? <input type="text" className={`${styles.cellInput} ${styles.centerCell}`} value={rtd.temp_dalam || ''} onChange={e => setTempRow('rtd', 'temp_dalam', e.target.value)} style={{ width: '40px' }} /> : <span>{rtd.temp_dalam || '-'}</span>}
                <span style={{ marginLeft: '2px' }}>°C</span>
              </div>
            </td>
            <td colSpan={2} className={styles.centerCell}>{isEdit ? <input type="text" className={`${styles.cellInput} ${styles.centerCell}`} value={rtd.digital_type || ''} onChange={e => setTempRow('rtd', 'digital_type', e.target.value)} /> : (rtd.digital_type || '-')}</td>
            <td colSpan={5}>{isEdit ? <textarea className={styles.cellInput} style={{ minHeight: '30px', resize: 'vertical' }} value={rtd.setting || ''} onChange={e => setTempRow('rtd', 'setting', e.target.value)} /> : <div style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word', maxHeight: '60px', overflow: 'hidden', fontSize: '7px' }}>{rtd.setting || '-'}</div>}</td>
          </tr>
          ...
          <tr>
            ...
            <td colSpan={6} style={{ padding: '3px 8px', fontSize: '9px' }}>
              {isEdit ? <textarea className={styles.cellInput} style={{ width: '100%', minHeight: '30px', resize: 'none', border: 'none', fontSize: '9px' }} value={data.cooling_system?.left?.condensor || ''} onChange={e => setCooling('left','condensor',e.target.value)} /> : <div style={{ minHeight: '30px', fontSize: '9px' }}>{data.cooling_system?.left?.condensor}</div>}
            </td>
            <td className={styles.centerCell} style={{ fontWeight: 'bold' }}>Condensor<br/><span style={{fontWeight:'normal',fontSize:'6px'}}>散熱器</span></td>
            <td colSpan={6} style={{ padding: '3px 8px', fontSize: '9px' }}>
              {isEdit ? <textarea className={styles.cellInput} style={{ width: '100%', minHeight: '30px', resize: 'none', border: 'none', fontSize: '9px' }} value={data.cooling_system?.right?.condensor || ''} onChange={e => setCooling('right','condensor',e.target.value)} /> : <div style={{ minHeight: '30px', fontSize: '9px' }}>{data.cooling_system?.right?.condensor}</div>}
            </td>
          </tr>
          ...
          <tr>
            ...
            <td colSpan={6} style={{ padding: '3px 8px', fontSize: '9px' }}>
              {isEdit ? <textarea className={styles.cellInput} style={{ width: '100%', minHeight: '30px', resize: 'none', border: 'none', fontSize: '9px' }} value={data.cooling_system?.left?.fan_antimist_notes || ''} onChange={e => setCooling('left','fan_antimist_notes',e.target.value)} /> : <div style={{ minHeight: '30px', padding: '2px 0', fontSize: '9px' }}>{data.cooling_system?.left?.fan_antimist_notes}</div>}
            </td>
            <td className={styles.centerCell} style={{ fontWeight: 'bold' }}>Fan Antimist<br/><span style={{fontWeight:'normal',fontSize:'6px'}}>風扇</span></td>
            <td colSpan={6} style={{ padding: '3px 8px', fontSize: '9px' }}>
              {isEdit ? <textarea className={styles.cellInput} style={{ width: '100%', minHeight: '30px', resize: 'none', border: 'none', fontSize: '9px' }} value={data.cooling_system?.right?.fan_antimist_notes || ''} onChange={e => setCooling('right','fan_antimist_notes',e.target.value)} /> : <div style={{ minHeight: '30px', padding: '2px 0', fontSize: '9px' }}>{data.cooling_system?.right?.fan_antimist_notes}</div>}
            </td>
          </tr>
        </tbody>
      </table>
        ...
        <div className={styles.rightInfoArea}>
          ...
          <div className={styles.qcChecklistBox} style={{ background: '#fff', border: '1px solid #000' }}>
            ...
            {[
              ...
              <label key={item.key} className={styles.qcLabel}>
                {!isEdit ? (
                  <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '13px', height: '13px', minWidth: '13px', border: data.performance_inspection?.[item.key] ? '2px solid #2e5bff' : '2px solid #94a3b8', borderRadius: '2px', background: data.performance_inspection?.[item.key] ? '#2e5bff' : '#fff', flexShrink: 0 }}>
                    {data.performance_inspection?.[item.key] && <svg width="8" height="8" viewBox="0 0 8 8" fill="none"><path d="M1.5 4L3.5 6L6.5 2" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                  </span>
                ) : (
                  <input type="checkbox" checked={data.performance_inspection?.[item.key] || false} onChange={e => setPerf(item.key, e.target.checked)} />
                )}
            ...
            <label className={styles.qcLabel}>
              {!isEdit ? (
                <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '13px', height: '13px', minWidth: '13px', border: data.performance_inspection?.qc_2 ? '2px solid #2e5bff' : '2px solid #94a3b8', borderRadius: '2px', background: data.performance_inspection?.qc_2 ? '#2e5bff' : '#fff', flexShrink: 0 }}>
                  {data.performance_inspection?.qc_2 && <svg width="8" height="8" viewBox="0 0 8 8" fill="none"><path d="M1.5 4L3.5 6L6.5 2" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                </span>
              ) : (
                <input type="checkbox" checked={data.performance_inspection?.qc_2 || false} onChange={e => setPerf('qc_2', e.target.checked)} />
              )}
              <span>Suhu tercapai dalam <span style={{ margin: '0 4px' }}>{isEdit ? <input type="text" className={styles.inlineInput} style={{ width: '30px', margin: '0 4px', borderBottom: '1px dotted #000' }} value={data.performance_inspection?.qc_2_time || ''} onChange={e => setPerf('qc_2_time', e.target.value)} /> : (data.performance_inspection?.qc_2_time || '___')}</span> menit</span>
            </label>
          </div>
        </div>
      </div>
